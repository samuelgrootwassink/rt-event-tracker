...
class NER():
    ...
    def common_entity_sets(self, named_entities:list, similarity = 0.8, minimum_set_length = 1):
        """
        ...
        for nes in common_entities_dict:
            if not nes or len(nes) <= minimum_set_length:
                continue
            
            for ne in named_entities:
                current_similarity = max(len(ne.intersection(nes)), 1) / len(nes)
                if nes == ne :
                    common_entities_dict[nes] += weight
                elif current_similarity >= similarity:
                    common_entities_dict[nes] = round( common_entities_dict[nes] + (weight * (current_similarity - similarity + .25)), 2)
